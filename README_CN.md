...
2. 如果启用了 `add_custom_import_hook`，将会生成加载器桩 (`.py` 文件)（如果 `keep_original` 为 false，可能会覆盖原始文件）。将此加载器桩和对应的 `.pye` 文件一起放在您的自定义节点目录中。
3. `LmcqCodeDecryptionLoader` 节点可用于测试加载加密文件，但通常情况下，Python 的导入机制会通过加载器桩自动处理加载。
